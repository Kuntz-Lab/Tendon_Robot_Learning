...
e_v = np.loadtxt('./csv1/history_eight_50_real.txt')
width_list = np.loadtxt('./csv1/history_eight_50_real_width.txt')
height_list = np.loadtxt('./csv1/history_eight_50_real_height.txt')
...
for i in range(10):
  ...
  if True:
    ...
    for iter_num in range(50):
        ...
        x_center = 0 #0 + 0.05
        z_center = 0.18
        start_x = np.random.uniform(x_center - 0.02, x_center + 0.02)  # sphere with 0.055, circle with 0.05
        start_y = -0.02   # sphere with -0.10 while circle with -0.14
    ...
    if(exp_df.mean() == min(exp_mean)):
        import time
        saved_model_path = "./real_saved_models/eight_50_train_"+str(int(time.time()))
