...
def main(arguments):
    ...
    for iter_num in range(1):
        ...
        x_center = 0 #0 + 0.05
        z_center = 0.07
        start_x = 0.005 #np.random.uniform(x_center - 0.04, x_center + 0.04)  
        start_y = -0.12 
        start_z = 0.0607571 #np.random.uniform(z_center - 0.02 , z_center + 0.02)
        radius_min = 0.01
        radius_max = 0.04
    ...
    exp_current_list.append(exp_current)
    avg_current_list.append(avg_current)
    print(exp_current_list)
    print(avg_current_list)
    for i in range(int(num_data.shape[0]/round_num)):
        num_current = num_data[i*round_num:i*round_num+round_num, :]
        num_current_0_list = copy.deepcopy(num_current)
        for j in range(round_num):
            num_current[j,0] = num_current[j,0] + start_x + width*2 - num_current_0_list[0][0]
            num_current[j,1] = num_current[j,1] + start_y - num_current_0_list[0][1]
            num_current[j,2] = num_current[j,2] + start_z - num_current_0_list[0][2]
        avg_current[0,:] = [start_x + width*2, start_y, start_z]#num_data[i*round_num, :]
        for j in range(1,round_num):
            avg_current[j,:] = avg_current[0,:] + mean_change[j-1]
        num_current_list.append(num_current)
        avg_current_list.append(avg_current)
        #exp_df.append(similaritymeasures.frechet_dist(exp_current, avg_current).tolist())
        num_df.append(similaritymeasures.frechet_dist(num_current, avg_current).tolist())
    # print(num_current_list)
    # print(avg_current_list)
    print(exp_df)
    print(num_df)
