...
inline TipForces calc_tip_forces(
                                        ...
                                        double s_start) const
{
  ...
  auto [v0, u0, iters] = solve_initial_bending(v_guess, u_guess,
      tendons, tau, Ks.K_bt_inv, Ks.K_se_inv, 30, 1e-5, 1e-6, s_start,
      rs, cache);
  UNUSED_VAR(iters);
  res.u_i = u0;
  res.v_i = v0;
                                              ...
                                              double s_start) const
{
  ...
  //     "  v_guess:    [" << v_guess.transpose() << "]\n"
  //     "  u_guess:    [" << u_guess.transpose() << "]\n";
  auto [v0, u0, iters] = solve_initial_bending_unopt(v_guess, u_guess,
      tendons, tau, Ks.K_bt_inv, Ks.K_se_inv, 30, 1e-5, 1e-6, s_start);
  UNUSED_VAR(iters);
  res.u_i = u0;
  res.v_i = v0;
...
TipForces TendonRobot::tip_forces(
    const std::vector<double> &tau, const TendonResult &result) const
{
  auto rs = get_r_info(tendons, specs.L);
  auto Ks = get_stiffness_matrices(specs);
  return calc_tip_forces(tau, result.R.back(), result.u_f, result.v_f,
    ...
    ) const
{
  ...
  IntFunc integrate = [&](double *vu, double *FL_e_out, int vu_dim, int FL_e_dim) {
    ...
    auto tip_forces = calc_tip_forces(tau, R, u, v, Ks.K_se, Ks.K_bt, rs.r, rs.r_dot);
  ...
  // calculate initial conditions
  double vu[6] {
    0, 0, 1,  // initial v guess = v* = z_hat
    0, 0, 0   // initial u guess = u* = 0
  };
  ...
  //    rs, cache);
  //UNUSED_VAR(iters);
  //std::copy_n(v0.data(), 3, vu);
  //std::copy_n(u0.data(), 3, vu+3);
  ...
  if (verbose) {
    ...
    switch (int(info[6])) {
      ...
      case 5: term_expl = "progres not possible (recommend restart and increasing mu)"; break;
    ...
    std::cout <<
      ...
      "  initial state:\n"
      "    u initial:        [" << u0.transpose() << "]\n" //[0, 0, 0]\n"
      "    v initial:        [" << v0.transpose() << "]\n" //[0, 0, 1]\n"
      "  initial estimates:\n"
