...
void def_class_TendonResult(py::module &m) {
  py::class_<TendonResult>(m, "TendonResult",
    ...
    .def(py::init<std::vector<double>,
        ...
        py::arg("u_i") = E::Vector3d{},
        py::arg("v_i") = E::Vector3d{},
        py::arg("u_f") = E::Vector3d{},
        py::arg("v_f") = E::Vector3d{})
...
void def_class_TendonRobot(py::module &m) {
  py::class_<TipForces>(m, "TipForces",
      "Tip forces calculated from a shape computation")
    ...
    // attributes
    .def_readwrite("F_e", &TipForces::F_e,
                   "External force exterted on the robot tip")
    .def_readwrite("L_e", &TipForces::L_e,
                   "External torques exterted on the robot tip")
    .def_readwrite("F_t", &TipForces::F_t,
                   "Force on the robot tip by the tendons")
    .def_readwrite("L_t", &TipForces::L_t,
                   "Torque on the robot tip by the tendons")
    .def_readwrite("n", &TipForces::n,
                   "internal force carried by the backbone")
    .def_readwrite("m", &TipForces::m,
                   "internal torque carried by the backbone")

    // python-specific added methods
    .def("__copy__", [](const TipForces &obj) {
          return TipForces(obj);
        })
    .def("__deepcopy__", [](const TipForces &obj) {
          return TipForces(obj);
        })
    .def(py::pickle(
        [](const TipForces &v) { // __getstate__
  ...
  py::class_<TendonRobot>(m, "TendonRobot",
    ...
    .def(py::init<double, BackboneSpecs,
                  std::vector<TendonSpecs>, bool, bool>(),
        py::arg("r") = 0.015,
    ...
    .def("tip_forces", &TendonRobot::tip_forces,
        py::arg("tau"), py::arg("result"),
        "Returns the TipForces at the tip from the tensions and shape")
    .def("is_valid",
        py::overload_cast<const std::vector<double>&,
                          const TendonResult&>(
